...
function getLocation(location)
{
  ...
  if (location.city)
    locationList.push(location.city)
  if (location.province)
    locationList.push(location.province)
  if (location.country)
    locationList.push(location.country)
  let ans = ""
  locationList.forEach((loc, ind) => {
    ans += loc
    if (ind != locationList.length - 1)
      ans += ", "
  })
  return ans
}
...
function Hotel({hotel}) {
  ...
  return (
    <Box
      ...
      maxW="100%"
    >
      <Image src={hotel.imgsrc} width="90%" height="60%" borderRadius="20px" objectFit="cover"/>
      <Text fontWeight="bold">{hotel.name}</Text>
      <Text>{getLocation(hotel.location)}</Text>
      <Grid templateColumns='repeat(5, 1fr)' gap={1} mt={4}>
        <Image src={"../../assets/stars/" + getStar(rating, 0) + ".png"} width="30px"/>
        <Image src={"../../assets/stars/" + getStar(rating, 1) + ".png"} width="30px"/>
        <Image src={"../../assets/stars/" + getStar(rating, 2) + ".png"} width="30px"/>
        <Image src={"../../assets/stars/" + getStar(rating, 3) + ".png"} width="30px"/>
        <Image src={"../../assets/stars/" + getStar(rating, 4) + ".png"} width="30px"/>
      </Grid>
